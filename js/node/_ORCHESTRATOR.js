...
proto.createComponent = function(componentName,args,mods) {
    ...
    switch (componentName) {
        ...
        case 'chopper':
            ...
            settings.args.push( pick( args[1], {mod: 1, min: 0, max: 2, floor: 0, ceil: 1  }) ); // depth
        ...
        case 'movement':
            ...
            switch (speed) {
                case 'slow':
                    ...
                    if (mod==='fudgeChance') {
                        args = [tombola.range(4,8),tombola.rangeFloat(0.005,0.015),tombola.range(10000,30000)];
                    }
                    if (mod==='rangeChance') {
                        args = [tombola.rangeFloat(-0.08,0.08),tombola.range(10000,25000)];
                    }
                    break;
                case 'medium':
                    if (mod==='moveTo' || mod==='weave' || mod==='glide' || mod==='glide2') {
                    ...
                    if (mod==='fudgeChance') {
                        args = [tombola.range(4,8),tombola.rangeFloat(0.005,0.015),tombola.range(4000,10000)];
                    }
                    if (mod==='rangeChance') {
                        args = [tombola.rangeFloat(-0.08,0.08),tombola.range(4000,10000)];
                    }
                    break;
                case 'fast':
                    if (mod==='moveTo' || mod==='weave' || mod==='glide' || mod==='glide2') {
                    ...
                    if (mod==='fudgeChance') {
                        args = [tombola.range(4,8),tombola.rangeFloat(0.005,0.015),tombola.range(2000,4000)];
                    }
                    if (mod==='rangeChance') {
                        args = [tombola.rangeFloat(-0.08,0.08),tombola.range(2000,4000)];
                    }
                    break;
            }
            break;
